		...
		return new Promise(async (resolve, reject) => {
			try {
				request(request_options, function (err, response, body) {
					console.log("debug", `OMFG::${JSON.stringify(err)}::${JSON.stringify(response)}::${JSON.stringify(body)}`)
					if (err) {
						return reject(err);
					}
					try {
						...
						if (response.statusCode !== 200) {
							return reject(new Error(`Got a non http 200 header with response ${result}`));
						}
						...
						return reject(new Error(`Error: Got an unexpected response from API: ${body}`));
					}
				});
			} catch (err) {
				return reject(err);
